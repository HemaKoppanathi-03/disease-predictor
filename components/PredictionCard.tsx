...
const RiskGauge: React.FC<{ probability: number, label: string }> = ({ probability, label }) => {
  ...
  return (
    <div className="relative w-48 h-24 overflow-hidden mx-auto">
      <div className="absolute w-48 h-48 rounded-full border-[24px] border-slate-200 top-0"></div>
      <div
      ...
      ></div>
      <div className="absolute w-48 h-48 rounded-full border-t-[24px] border-transparent border-slate-200 top-0 transform rotate-[-90deg]"></div>
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        <div className="text-center md:text-left">
          <h2 className="text-sm font-semibold uppercase text-blue-600 tracking-wide">Primary Risk Assessment</h2>
          <p className="text-3xl font-extrabold text-slate-800 my-2">{prediction.disease}</p>
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="mt-2 text-sm text-slate-600">Risk Probability: <span className="font-bold">{(prediction.riskProbability * 100).toFixed(0)}%</span></p>
        </div>
      </div>
      <div className="mt-8 pt-6 border-t border-slate-200">
        <h3 className="text-lg font-bold text-slate-800 mb-3">Key Contributing Factors</h3>
